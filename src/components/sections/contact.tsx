...
function buildWhatsAppMessage(form: ContactFormState): string {
  return [
    "Hola Nexo Studio, quiero iniciar una conversación sobre un proyecto.",
    `Nombre: ${form.name}`,
    `Teléfono: ${form.phone}`,
    `Email: ${form.email}`,
...
export function Contact() {
  ...
  return (
    <section id="contacto" className="py-20 sm:py-24 lg:py-32 bg-dark-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeader
          eyebrow="Iniciar un proyecto"
          title="Cuéntanos qué quieres"
          highlight="crear"
          description="Queremos entender el proyecto antes de hablar de una propuesta. Compártenos ubicación, alcance, inversión estimada y momento de inicio."
        />
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-stretch">
          <FadeIn direction="left" className="relative min-h-[380px] sm:min-h-[420px] lg:min-h-[460px]">
            <div className="relative h-full min-h-[380px] sm:min-h-[420px] lg:min-h-[460px] rounded-3xl overflow-hidden">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-dark-900/88 via-dark-900/15 to-transparent" />
            </div>

            <div className="absolute bottom-5 left-5 right-5 bg-dark-900/84 backdrop-blur-md rounded-2xl border border-white/10 p-5 sm:p-6">
              <p className="text-xs uppercase tracking-[0.18em] text-warm mb-4">
                Nexo Studio · Medellín
              </p>
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-sm font-medium">
                      {contact.phonePrimaryDisplay} / {contact.phoneSecondaryDisplay}
                    </div>
                  </div>
                </a>
          ...
          <FadeIn direction="right" delay={0.15}>
            <div className="h-full bg-dark-900/55 border border-white/6 rounded-3xl p-6 sm:p-8 lg:p-9">
              <h3 className="text-xl sm:text-2xl font-semibold text-white mb-2">
                Evaluemos tu proyecto
              </h3>
              <p className="text-white/60 text-sm mb-6 sm:mb-7">
                La información nos ayuda a llegar a la primera conversación con contexto y a determinar el alcance adecuado.
              </p>
              ...
              {submitted ? (
                ...
                >
                  ...
                  <p className="text-white/52 text-sm">
                    Revisa la ventana de WhatsApp para enviar la información.
                  </p>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        required
                        className="bg-dark-700/55 border-white/12 text-white placeholder:text-white/50 focus:border-warm/55 h-11 rounded-xl"
                      />
                    </div>
                    <div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormState((s) => ({ ...s, area: e.target.value }))}
                        className="bg-dark-700/55 border-white/12 text-white placeholder:text-white/50 focus:border-warm/55 h-11 rounded-xl"
                      />
                    </div>
                    <div>
                      <label htmlFor="budget" className="block text-sm font-medium text-white/65 mb-1.5">Inversión</label>
                      <select
                        id="budget"
                        style={selectStyle}
                    ...
                    <div>
                      <label htmlFor="timeline" className="block text-sm font-medium text-white/65 mb-1.5">Inicio</label>
                      <select
                        id="timeline"
                        style={selectStyle}
                  ...
                  <div>
                    <label htmlFor="message" className="block text-sm font-medium text-white/65 mb-1.5">Cuéntanos sobre el proyecto</label>
                    <Textarea
                      id="message"
                      placeholder="¿Qué propiedad es, qué quieres transformar y qué es importante para ti?"
                      rows={4}
                      value={formState.message}
                      onChange={(e) => setFormState((s) => ({ ...s, message: e.target.value }))}
                      required
                      className="bg-dark-700/55 border-white/12 text-white placeholder:text-white/50 focus:border-warm/55 rounded-xl resize-none"
                    />
                  </div>
                  ...
                  {popupBlocked && (
                    <div className="mt-3 p-4 rounded-xl bg-warm/12 border border-warm/30 text-sm">
                      ...
                      <p className="text-white/70 mb-3">Puedes continuar directamente aquí:</p>
                      <a
