...
export async function testCrmWrite(): Promise<boolean> {
  if (!isCrmConfigured()) return false;
  ...
  const rows = await sql`
    INSERT INTO nexo_leads (
      name, phone, email, project_type, location, budget, timeline, message,
      status, priority, source, utm_source, utm_medium, utm_campaign
    )
    VALUES (
      'NEXO CRM Health Check',
      '0000000000',
      'health-check@nexostudioarq.com',
      'Prueba técnica',
      'Preview',
      '$100M - $250M COP',
      'health-check',
      'Registro temporal de validación CRM',
      'new',
      'medium',
      'healthcheck',
      'internal_test',
  ...
  const found = Number((verify[0] as { count?: number } | undefined)?.count) === 1;

  await sql`
    DELETE FROM nexo_leads
    WHERE id = ${id}::uuid
  `;
